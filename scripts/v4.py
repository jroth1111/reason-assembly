...
class CoFailureStore:
    ...
    def record(self, *, routes: Iterable[str], families: Iterable[str],
        ...
        row = data["buckets"].setdefault(key, {
            ...
            "route_correct": {}, "high_risk": task_kind in {"safety_gate", "implementation"},
            "routes": sorted(set(routes)), "families": sorted(set(families)),
            "task_kind": task_kind, "domain": domain, "answer_format": answer_format,
        })
        ...
        for route, status in route_correct.items():
            row["route_correct"][route] = row["route_correct"].get(route, 0) + float(status)
        self._write(data)
...
def selective_judgment(*, scores_and_correct: list[tuple[float, bool]],
                       ...
                       independently_verified: bool) -> SelectiveJudgmentReceipt:
    ...
    cap = 1.0 if calibrated or evidence_resolved else 0.65
    ...
    return SelectiveJudgmentReceipt(
        ...
        judgment_risk=risk, threshold=threshold,
