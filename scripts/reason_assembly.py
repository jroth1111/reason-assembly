...
def outcome_command(args: argparse.Namespace) -> Outcome:
    ...
    if invalidated and store._target("claim-genealogy.json").exists():
        ...
        store.write_json("claim-genealogy.json", genealogy)
        store.write_json("taint-state.json", taint)
        if taint.tainted_ids and store._target("verdict.json").exists():
            ...
            store.write_json("verdict.json", current_verdict)
        if taint.tainted_ids and store._target("finality-certificate.json").exists():
            ...
            store.write_json("finality-certificate.json", certificate)
        if (
            taint.tainted_ids
            and store._target("rollout-card.json").exists()
            and store._target("finality-certificate.json").exists()
        ):
            card = RolloutCard.model_validate(store.read_json("rollout-card.json"))
            card.finality = FinalityCertificate.model_validate(
                store.read_json("finality-certificate.json")
            )
            card.taint_transitions = taint.transitions
            store.write_json("rollout-card.json", card)
        store.append_event(
            "post_outcome_invalidation",
    ...
    calibration = calibration_store.read()
    ...
    calibration_store.write(calibration)
    candidate_rows = {
