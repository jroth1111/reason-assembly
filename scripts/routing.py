...
def _preference_score(model_id: str) -> float:
    try:
...
def _role_fit(capability: ModelCapability, role: Role, task_kind: TaskKind) -> float:
    base = 0.55 + 0.35 * _preference_score(capability.id)
    ident = capability.id.lower()
    if task_kind in {"implementation", "review"} and any(
        key in ident for key in ("codex", "coder", "devstral", "sol")
    ):
        base += 0.1
    if role in {"judge", "validator", "integrator"} and capability.id == "gpt-5.6-sol":
        base += 0.1
    ...
    *,
    ...
    for item in catalogue:
        ...
        if role not in capability.roles or capability.family in excluded:
            continue
        if capability.id == "gpt-5.6-luna" and role != "utility":
            continue
        effort = (
            item.effort
        ...
        fit = _role_fit(capability, role, task_kind)
...
def candidate_pool(
    ...
    *,
    ...
    for capability in ordered:
        if role not in capability.roles or capability.id in {
            "gpt-5.6-sol",
            "gpt-5.6-luna",
        }:
            continue
