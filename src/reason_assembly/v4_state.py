...
class PrivateJsonStore:
    ...
    def _initialize_unlocked(self, data: dict[str, Any]) -> bool:
    ...
    def initialize(self, data: dict[str, Any]) -> bool:
        with flock_exclusive(self.lock_path):
            return self._initialize_unlocked(data)
    ...
    def locked_write(
        ...
        """Read, mutate, and atomically replace this schema-v4 store under one lock."""
        with flock_exclusive(self.lock_path):
            data = self.read()
            replacement = mutator(data)
            result = data if replacement is None else replacement
...
class AnchorStore:
    ...
    def import_file(self, path: Path) -> list[CalibrationAnchor]:
        ...
        def merge(data: dict[str, Any]) -> None:
            ...
            for anchor in incoming:
                if (
                    anchor.id in existing
                    and existing[anchor.id]["sha256"] != anchor.sha256
                ):
                    raise ValueError(
                        f"anchor id already exists with different content: {anchor.id}"
                    )
                existing[anchor.id] = anchor.model_dump(mode="json")
            data["anchors"] = [existing[key] for key in sorted(existing)]

        self.store.locked_write(merge)
        return incoming
    ...
    def retire(self, anchor_id: str) -> CalibrationAnchor:
        retired: list[CalibrationAnchor] = []
        ...
        def mutate(data: dict[str, Any]) -> None:
            for row in data["anchors"]:
                if row["id"] == anchor_id:
                    ...
                    retired.append(CalibrationAnchor.model_validate(row))
                    return
            raise ValueError(f"anchor not found: {anchor_id}")

        self.store.locked_write(mutate)
        return retired[0]
...
class RouteEpochStore:
    ...
    def current(self, catalogue: list[dict]) -> RouteEpoch:
        ...
        selected: list[RouteEpoch] = []

        def mutate(data: dict[str, Any]) -> None:
            for row in data["epochs"]:
                if row["catalogue_fingerprint"] == fingerprint:
                    selected.append(RouteEpoch.model_validate(row))
                    return
            epoch = RouteEpoch(
