...
def run_command_verifier(
    ...
    *,
    ...
    extra_path = os.environ.get("REASON_ASSEMBLY_VERIFY_PATH", "")
    safe_env = {
        "PATH": os.pathsep.join(filter(None, (extra_path, os.defpath))),
        "LANG": "C.UTF-8",
        "LC_ALL": "C.UTF-8",
        "NO_PROXY": "*",
        "HTTP_PROXY": "",
    ...
    shell_enabled = step.shell if allow_shell is None else allow_shell
    try:
        command: str | list[str]
        kwargs: dict[str, object] = {}
        if shell_enabled:
            command = step.executor_input
            kwargs = {"shell": True, "executable": command_shell()}
        else:
            command = shlex.split(step.executor_input)
            if not command:
        ...
        message = str(exc)
        if not shell_enabled and any(marker in step.executor_input for marker in ("|", ";", "&&", ">")):
            message += "; shell syntax requires the explicit --verify-shell CLI option"
        return VerificationReceipt(
            step_id=step.id,
            claim_id=step.claim_id,
            kind=step.kind,
            status="inconclusive",
            executor="engine-command",
            output_sha256=sha256_text(message),
            observation=message,
            evidence_refs=step.evidence_refs,
            deterministic=True,
            ambiguity=message,
