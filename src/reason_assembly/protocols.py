...
def extract_json(text: str) -> Any:
    """Extract one unambiguous JSON value without greedy brace guessing."""
    stripped = text.strip()
    ...
    for index, character in enumerate(stripped):
        ...
        try:
            ...
            continue
        if stripped[index + end :].strip() and stripped[index + end :].lstrip().startswith(("{", "[")):
            continue
...
class CouncilEngine:
    def __init__(
        ...
        *,
        ...
        self.routing_policy = load_routing_policy(self.state)
    ...
    async def preflight(
        ...
        def preferred_route(route_role: str, configured: str | None) -> Route | None:
            explicit = self._override(route_role) or configured
            if explicit:
                return fixed_route(self.catalogue, explicit, route_role)  # type: ignore[arg-type]
            rows = select_role_routes(
                self.catalogue,
                role=route_role,  # type: ignore[arg-type]
        ...
        judge = preferred_route("judge", self.routing_policy.judge_model)
        if judge is None:
            raise RuntimeError("no catalogue route can serve as judge")
        luna = preferred_route("utility", self.routing_policy.luna_model)
        integrator = (
            preferred_route("integrator", self.routing_policy.integrator_model)
            if role == "worker"
            else None
        )
    ...
    async def model_verifications(
        ...
        for step in command_steps:
            allow_shell = (
                step.shell
                and self.request.verify_shell
                and step.executor_input in self.request.verify_commands
            )
            receipt = run_command_verifier(step, cwd=cwd, allow_shell=allow_shell)
        ...
        try:
            ...
            if self.request.verify_commands:
                command_plan = build_verification_plan(
                    ...
                    allow_shell=self.request.verify_shell,
                )
                command_plan.steps = [
