...
class WorkerBackend(Protocol):
    def preflight(self, settings: ProxySettings) -> None: ...
...
class SubprocessWorkerBackend:
    def executable(self, settings: ProxySettings) -> str:
        return str(getattr(settings, "worker_executable", "codex"))

    def preflight(self, settings: ProxySettings) -> None:
        configured = self.executable(settings)
        executable = shutil.which(configured)
        if executable is None:
            raise WorkerUnavailableError(
                f"worker executable {configured!r} was not found; install it or set "
                "REASON_ASSEMBLY_WORKER to an executable path"
            )
        try:
            result = subprocess.run(
                [executable, "--version"],
            ...
            raise WorkerUnavailableError(
                f"worker executable {configured!r} failed its --version preflight: {exc}"
            ) from exc
        if result.returncode:
            detail = (result.stderr or result.stdout).strip()[:300]
            raise WorkerUnavailableError(
                f"worker executable {configured!r} failed its --version preflight"
    ...
    def invoke(
        ...
        return _invoke_subprocess_worker(
            repo, route, settings, prompt, timeout, self.executable(settings)
        )
...
def preflight_worker(
    ...
    (backend or SubprocessWorkerBackend()).preflight(settings)


def _invoke_subprocess_worker(
    repo: Path,
    route: Route,
    settings: ProxySettings,
    prompt: str,
...
def invoke_codex(
    ...
    """Compatibility entry point for the default or injected worker backend."""
    return (backend or SubprocessWorkerBackend()).invoke(
        repo, route, settings, prompt, timeout
    )
    ...
    if not failure and mode == "regression":
        if not commands:
            ...
            exit_code, output, timed_out = invoke_codex(
                worktree, route, settings, prompt, timeout, worker_backend
            )
            logs.append(output)
...
class ImplementationEngine:
    def __init__(
        ...
        *,
        ...
        self.worker_backend = worker_backend or SubprocessWorkerBackend()
        ...
        council_request = CouncilRequest(
            ...
            verify_shell=request.verify_shell,
...
async def run_implementation(
    ...
    *,
    ...
    resolved_settings = settings or ProxySettings()
    resolved_backend = worker_backend or SubprocessWorkerBackend()
    preflight_worker(resolved_settings, resolved_backend)
