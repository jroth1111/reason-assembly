...
class ReliabilityStore:
    ...
    def _load_raw(self) -> ReliabilitySnapshot:
        if not self.path.exists():
            return self.empty()
        raw = json.loads(self.path.read_text())
        raw.pop("pair_buckets", None)
        return ReliabilitySnapshot.model_validate(raw)
    ...
    def _write_unlocked(self, snapshot: ReliabilitySnapshot) -> None:
        payload = json.dumps(
    ...
    def update(
        ...
        """Merge an outcome into the latest snapshot under one read-modify-write lock."""
        del snapshot  # retained for source compatibility; the locked latest value wins
        with flock_exclusive(self.lock_path):
            return self._update_unlocked(self._load_raw(), outcome, attributions)
