...
from .state_compat import exclusive_state_lock, flock_exclusive
...
class RunStore:
    ...
    def write_transaction(
        ...
        *,
        require_absent: str | None = None,
        commit_artifact: str = "manifest.json",
    ) -> list[Path]:
        """Apply a bounded run-level transaction with the commit artifact written last."""
        rows = list(mutations)
        ordered = [row for row in rows if row[0] != commit_artifact]
        ordered.extend(row for row in rows if row[0] == commit_artifact)
        written: list[Path] = []
        with flock_exclusive(self.path / ".run.lock"):
            if require_absent and self._target(require_absent).exists():
                raise RuntimeError(
                    "outcome is already recorded; use revisit for corrected evidence"
                )
            for relative, value in ordered:
                written.append(self.write_json(relative, value))
        return written
    ...
    def _artifact_hashes(self) -> dict[str, str]:
        return {
            name: hashlib.sha256(self._target(name).read_bytes()).hexdigest()
            for name in self.artifact_names()
            if name != "manifest.json"
        }

    def seal_manifest(self) -> str:
        """Seal current non-manifest artifacts without storing private key material."""
        with flock_exclusive(self.path / ".run.lock"):
            ...
            manifest["artifacts"] = self.artifact_names()
            self.write_json("manifest.json", manifest)
        return digest

    def verify_integrity(self) -> list[str]:
        """Return stable diagnostics for unsealed, missing, added, or modified artifacts."""
        manifest = self.read_json("manifest.json")
        expected = manifest.get("artifact_hashes") or {}
        sealed = manifest.get("integrity_sha256")
        if not sealed or not expected:
            return ["manifest not sealed"]
        expected_digest = hashlib.sha256(
            json.dumps(expected, sort_keys=True, separators=(",", ":")).encode()
        ).hexdigest()
        mismatches = []
        if expected_digest != sealed:
            mismatches.append("manifest integrity digest")
        actual = self._artifact_hashes()
